...
avg_value_df = spark.sql("""
    SELECT ROUND(AVG(totalPrice), 2) AS avg_transacction_value    
    FROM samples.bakehouse.sales_transactions                     
""")
...
customer_summary_df = customers_df.select(
    customers_df.customerID.cast("string"),
    customers_df.first_name.cast("string"),
    customers_df.last_name.cast("string"),
    customers_df.city.cast("string")
)
...
avg_transaction_by_franchise_df = (transactions_df
    .groupBy(col("franchiseID"))  # Group by franchiseID
...
reviews_with_words_df = reviews_df.withColumn("review_words", split(col("review"), " "))
...
customers_without_reviews_df = (customers_df
    .join(
        reviews_df,
        customers_df["customerID"] == reviews_df["franchiseID"],  
        "left"
...
franchise_metrics_df = (
    ...
    .agg(
        ...
        avg(col("totalPrice")).alias("avg_transaction")
    )
