...
(transactions_df
 ...
 .format("delta")
 .mode("overwrite")
 .save(str(f"{working_dir}/transactions_partitioned"))
)
...
list_columns = ["customerID", "first_name", "last_name", "email_address"]

simple_dedup_df = dups_df.dropDuplicates(list_columns)  # List of column names
...
temp_columns = ["lcFirstName", "lcLastName", "lcEmail"]

# Drop duplicates based on normalized columns
deduped_df = normalized_df.dropDuplicates(temp_columns)  # List: ["lcFirstName", "lcLastName", "lcEmail"]
...
from pyspark.sql.functions import sum, count, countDistinct, desc, upper

# Step 1: Load deduplicated customers
clean_customers_df = (spark.read.format("delta")
    .load(f"{working_dir}/customers_deduplicated"))  # Load from f"{working_dir}/customers_deduplicated"

# # Step 2: Load and filter franchises to US only (predicate pushdown!)
usa_franchises_df = (
    spark.table("samples.bakehouse.sales_franchises")
    # Using upper() ensures we catch 'usa', 'Usa', or 'USA'
    .filter(upper(col("country")) == "US") 
)

# Debug line: Check if this actually returned anything
print(f"Franchises found: {usa_franchises_df.count()}")
